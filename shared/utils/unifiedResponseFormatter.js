 ...
 */
function formatCrossRefResponse(crossRefData) {
  ...
  const itemCount = crossRefData.message.items.length;
  console.log(`📊 CrossRef API レスポンス: ${itemCount}件受信`);
...
function formatGoogleBooksResponse(googleBooksData) {
  ...
  const results = googleBooksData.items.map(item => {
    ...
    const result = {
      ...
      url: volumeInfo.previewLink || volumeInfo.canonicalVolumeLink || '',
      source: 'Google Books',
      isBook,
      isBookChapter,
      bookTitle: '', // 書籍全体なので空
 ...
 */
function formatSemanticScholarResponse(semanticScholarData) {
 ...
 */
function formatNDLResponse(ndlData) {
...
/**
 * 出版社名から地名を除去 (CommonJS版)
 */
function cleanPublisherNameCJS(publisher) {
  ...
  // console.log(`📍 出版社名クリーニング (CommonJS): "${publisher}" → "${cleaned}"`);
 ...
 */
function formatCiNiiResponse(ciniiData) {
  ...
  const enhancedResults = ciniiData.results.map(item => {
    ...
    const cleanedPublisher = cleanPublisherNameCJS(rawPublisher);
...
// CommonJS exports for dev-proxy-simple.js compatibility
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    formatCrossRefResponse,
    formatGoogleBooksResponse,
    formatSemanticScholarResponse,
    formatNDLResponse,
    formatCiNiiResponse
  };
}

// ES6 exports for Vercel functions (commented out for CommonJS compatibility)
// export {
//   formatCrossRefResponse,
//   formatGoogleBooksResponse,
//   formatSemanticScholarResponse,
